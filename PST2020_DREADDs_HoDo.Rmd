---
title: "DREADDs PST2020 Homer Donut"
author: "CREW"
date: "22/02/2023"
output: 
  pdf_document:
...
# *** OLD DATA LOADINg PATHS. IGNORE IF USING RPROJ ***
...
[^n1]: The  first descriptive analyses were in Graph_PST2020choices.R - now in PST2020_DREADDs.rmd]. This version is an expanded version of PST2020_DREADDs_v2.RMD
...
data4B$Injection <- factor( data4B$Injection, levels = c("Vehicle","DCZ"))
...
agg.data4B.sub$Injection <- factor(agg.data4B.sub$Injection, levels = c("0", "no", "sham","DCZ"))
...
hom.chtype.glm <- glm(trial ~ Injection * choice_type , family = "poisson", data=subset(agg.data4B.sub, singe=="Homer" & choice_type != "Correct"))
summary(hom.chtype.glm)

Don.chtype.glm <- glm(trial ~ Injection * choice_type , family = "poisson", data=subset(agg.data4B.sub, singe=="Donut" & choice_type != "Correct"))
summary(Don.chtype.glm)

#Presentation fig CW


g1 <- ggplot(subset(agg.data4B.sub, singe=="Donut" & portes != "clear"), aes(x=as.factor(choice_type), y=trial   , fill=Injection))+
    geom_boxplot()+
...
g2 <- ggplot(subset(agg.data4B.sub, singe=="Homer" & portes != "clear"), aes(x=as.factor(choice_type), y=trial   , fill=Injection))+
    geom_boxplot()+
    scale_fill_manual(values=c("grey","chartreuse3"),labels = c("Vehicle", "DCZ"))+
    #ggtitle("First 25 trials")+
  theme(axis.text.x = element_text(color = "grey20"), axis.title.x = element_blank()) +
...
agg.data4B.sub.last$Injection <- factor(agg.data4B.sub.last$Injection, levels = c("0", "no", "sham","DCZ"))
...
hom.chtype.glm <- glm(trial ~ Injection * choice_type , family = "poisson", data=subset(agg.data4B.sub.last, singe=="Homer" & choice_type != "Correct" & portes != "clear"))
summary(hom.chtype.glm)

Don.chtype.glm <- glm(trial ~ Injection * choice_type , family = "poisson", data=subset(agg.data4B.sub.last, singe=="Donut" & choice_type != "Correct" & portes != "clear"))
summary(Don.chtype.glm)

hom.rep.glm <- glm(trial ~ Injection , family = "poisson", data=subset(agg.data4B.sub.last, singe=="Homer" & choice_type == "Repeat" & portes != "clear"))
summary(hom.chtype.glm)

Don.rep.glm <- glm(trial ~ Injection , family = "poisson", data=subset(agg.data4B.sub.last, singe=="Donut" & choice_type == "Repeat" & portes != "clear"))
summary(Don.chtype.glm)
